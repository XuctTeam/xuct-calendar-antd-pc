 ...
 * @LastEditTime: 2023-10-09 11:10:10
...
interface IInnerProps extends IPageOption {
  innerRef?: any
}

function Calendar(props: IInnerProps) {
  const {
    centerHeight,
    selectDay,
    fullCalendarLocal,
  ...
  const calendarOptions = {
    ...
    plugins: [dayGridPlugin, timeGridPlugin, interactionPlugin, rrulePlugin, listPlugin, bootstrap5Plugin],
    headerToolbar: {
  ...
  const dayCellContent = (item: any) => {
    ...
    return { html: `<p><label>${_dateF.cDay}</label><span>${_dateF.lunarFestival || _dateF.dayCn}</span></p>` }
  }
...
const RightCalendar = React.forwardRef((props: IPageOption, ref: Ref<any>) => <Calendar {...props} innerRef={ref} />)
RightCalendar.displayName = 'RightCalendar'
